...
class CustomViewfinderView (context: Context?, attrs: AttributeSet?): ViewfinderView(context,attrs) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        var previewFramingRect = cameraPreview.previewFramingRect
        if (framingRect != null && previewFramingRect != null) {
            this.framingRect = framingRect
            previewFramingRect = previewFramingRect
        }
        refreshSizes()
        if (framingRect == null || previewFramingRect == null) {
            return
        ...
        canvas.let { canvas ->
            if (resultBitmap != null) {
                ...
                if (currentPossible.isEmpty()) {
                    ...
                    paint.apply {
                        alpha = ViewfinderView.CURRENT_POINT_OPACITY
                ...
                currentLast?.let {
                    paint.apply {
                        alpha = ViewfinderView.CURRENT_POINT_OPACITY / 2
                        color = resultPointColor
                    }
                ...
                postInvalidateDelayed(
                    ViewfinderView.ANIMATION_DELAY,
                    frameLeft.toInt() - ViewfinderView.POINT_SIZE,
                    frameTop.toInt() - ViewfinderView.POINT_SIZE,
                    frameRight.toInt() + ViewfinderView.POINT_SIZE,
                    frameBottom.toInt() + ViewfinderView.POINT_SIZE
                )
            }
        }
    }
